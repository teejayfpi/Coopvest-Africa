  ...
  Future<void> _checkVerificationStatus() async {
    ...
    try {
      ...
      if (response['success'] == true && mounted) {
        setState(() { _isVerified = response['isVerified'] ?? false; });
    ...
    try {
      ...
      if (mounted) {
        if (response['success'] == true) {
          ...
          if (response['cooldownSeconds'] != null) _startCooldown(response['cooldownSeconds']);
        } else {
          setState(() => _errorMessage = response['error'] ?? 'Failed to send verification email');
          if (response['remainingSeconds'] != null) _startCooldown(response['remainingSeconds']);
        }
      }
    } catch (e) {
      if (mounted) setState(() => _errorMessage = 'Failed to send verification email');
    } finally {
      ...
      if (mounted) {
        if (response['success'] == true) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Email verified successfully!'), backgroundColor: CoopvestColors.success),
          );
          Future.delayed(const Duration(seconds: 2), () {
            if (mounted) Navigator.of(context).pushReplacementNamed('/home');
          });
        } else {
          setState(() => _errorMessage = response['error'] ?? 'Invalid verification code');
        }
      }
    } catch (e) {
      if (mounted) setState(() => _errorMessage = 'Verification failed');
    } finally {
            ...
            children: [
              Container(
                ...
                child: Icon(
                  ...
                  size: 50,
                  color: _isVerified ? CoopvestColors.success : CoopvestColors.primary,
                ),
              ),
              ...
              if (!_isVerified) ...[
                ...
                if (_errorMessage != null)
                  Padding(
                    ...
                    child: Text(_errorMessage!, style: const TextStyle(color: CoopvestColors.error, fontSize: 12)),
                  ),
                if (_successMessage != null)
                  Padding(
                    padding: const EdgeInsets.only(bottom: 8),
                    child: Text(_successMessage!, style: const TextStyle(color: CoopvestColors.success, fontSize: 12)),
                  ),
                ...
                SecondaryButton(
                  label: _cooldownSeconds > 0
                      ...
                      : 'Resend Email',
                  onPressed: (_cooldownSeconds > 0 || _isResending) ? () async {} : _resendVerificationEmail,
