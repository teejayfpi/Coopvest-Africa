...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Notify borrower that a guarantor confirmed via QR scan
      try {
        ...
        if (loan?.profile_id) {
          await notifyGuarantorResponded({
            ...
            guarantorName:      guarantor_name || 'A guarantor',
            action:             'accepted',
            loanId,
        ...
        logger.warn('Guarantor confirm notification failed (non-fatal):', notifyErr.message);
      }
