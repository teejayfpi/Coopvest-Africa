...
// ── Loan Guarantor Notifications ─────────────────────────────────────────────

/**
 * Notify a member that they have been listed as a guarantor for a loan.
 * Fires when a loan_guarantors record is created with status='pending'.
 *
 * @param {object} params
 * @param {string} params.guarantorProfileId  - UUID of the guarantor to notify
 * @param {string} params.borrowerName        - Display name of the loan applicant
 * @param {number} params.loanAmount          - Loan amount in NGN
 * @param {string} params.loanId              - UUID of the loan
 * @param {string} params.requestId           - UUID of the loan_guarantors record
 */
async function notifyGuarantorRequested({
  guarantorProfileId,
  borrowerName,
  loanAmount,
  loanId,
  ...
  const formatted = Number(loanAmount).toLocaleString('en-NG', { maximumFractionDigits: 0 });
  const title = 'Loan Guarantee Request';
  const body  = `${borrowerName} has listed you as a guarantor for a ₦${formatted} loan. Tap to review and respond.`;
  const type  = 'guarantor_request';
  const data  = {
    type,
  ...
  logger.info(`Guarantor request notification sent to ${guarantorProfileId} for loan ${loanId}`);
}

/**
 * Notify the loan borrower that one of their guarantors has responded.
 *
 * @param {object} params
 * @param {string} params.borrowerProfileId - UUID of the borrower to notify
 * @param {string} params.guarantorName     - Display name of the responding guarantor
 * @param {'accepted'|'declined'} params.action
 * @param {string} params.loanId
 * @param {number} params.guarantorsNow     - How many guarantors have now consented
 * @param {number} params.guarantorsRequired
 */
async function notifyGuarantorResponded({
  borrowerProfileId,
  guarantorName,
  action,
  loanId,
  ...
  const title    = accepted ? 'Guarantor Confirmed ✓' : 'Guarantor Declined';
  const progress = `${guarantorsNow}/${guarantorsRequired}`;
  const body     = accepted
    ? `${guarantorName} has accepted your loan guarantee request. Progress: ${progress} guarantors.`
    : `${guarantorName} has declined your loan guarantee request. You may need to find a replacement.`;
  const type     = accepted ? 'guarantor_accepted' : 'guarantor_declined';
  const data     = {
    type,
    loanId: String(loanId || ''),
    link:   '/loan-details',
  ...
  await Promise.all([
    sendInApp({ profileId: borrowerProfileId, title, body, type, priority: accepted ? 'normal' : 'high' }),
    pushToProfile({ profileId: borrowerProfileId, title, body, type, data }),
  ]);

  logger.info(`Guarantor response (${action}) notification sent to borrower ${borrowerProfileId} for loan ${loanId}`);
}
...
module.exports = {
  ...
  // Loan guarantors
  notifyGuarantorRequested,
  notifyGuarantorResponded,
  // Rollover
